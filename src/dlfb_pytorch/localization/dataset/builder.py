...
from dlfb_pytorch.localization.dataset import Dataset, Labels
from dlfb_pytorch.localization.dataset.loaders import ImageLoader, LabelLoader
from dlfb_pytorch.utils import validate_splits
...
class DatasetBuilder:
  ...
  def build(
    ...
    frames = self.labels.get_frames_of_random_proteins(n_proteins)
    ...
    set_ids = np.array(frames[exclusive_by].to_numpy(np.int32))
    shuffled_set_ids = np.random.permutation(np.unique(set_ids))

    frame_ids = np.array(frames["frame_id"].to_numpy(np.int32))
    ...
    for name, size in self._get_split_sizes(
      ...
      mask = np.isin(set_ids, shuffled_set_ids[start : (start + size)])
