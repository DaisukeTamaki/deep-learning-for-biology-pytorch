...
class Labels:
  ...
  def get_frames_of_random_proteins(
    self, max_proteins: int
  ) -> pd.DataFrame:
    all_proteins = self.protein_lookup()
    sample_idx = np.random.choice(
      all_proteins.shape[0], max_proteins, replace=False
    )
...
class Dataset:
  ...
  def get_dummy_input(self):
    import torch
    return torch.ones(1, 1, 100, 100)

  def get_random_annotated_frames(
    self, n: int, gene_symbols: list[str] = []
  ) -> list["AnnotatedFrame"]:
    from dlfb_pytorch.localization.dataset.utils import summarize_localization
    ...
    selected_frame_ids = np.random.choice(frame_ids, n, replace=False)
    lookup = (
  ...
  def plot_random_frames(
    ...
    for i, af in enumerate(
      self.get_random_annotated_frames(n, gene_symbols)
    ):
