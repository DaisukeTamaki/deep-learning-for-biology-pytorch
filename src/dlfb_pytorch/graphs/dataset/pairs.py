...
class Pairs:
  ...
  def get_eval_batches(
    ...
    indices = torch.arange(self._n_pairs())
  ...
  def get_train_batches(
    ...
    indices = torch.randperm(self._n_pairs())
    neg_sample = self._global_negative_sampling()
  ...
  def _global_negative_sampling(self) -> torch.Tensor:
    """Samples negative pairs from the entire set to match positive set size."""
    perm = torch.randperm(self.neg.shape[0])[: self.pos.shape[0]]
    return self.neg[perm]

  def get_dummy_input(self) -> dict[str, torch.Tensor]:
